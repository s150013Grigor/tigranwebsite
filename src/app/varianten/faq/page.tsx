          ...
          {faqs.map((f, i) => (
            <div key={i} className="border border-white/10 rounded-lg overflow-hidden">
              <button onClick={() => setOpen(open === i ? null : i)} className="w-full flex items-center justify-between p-5 text-left">
                ...
                {open === i ? <FaMinus className="text-white/50 flex-shrink-0" /> : <FaPlus className="text-white/50 flex-shrink-0" />}
              </button>
              <AnimatePresence>
                {open === i && (
                  <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                    <p className="px-5 pb-5 text-white/60 text-sm">{f.a}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
    ...
    <section className="bg-primary py-16">
      <div className="max-w-7xl mx-auto px-8 grid md:grid-cols-2 gap-16 items-start">
        <div className="sticky top-24">
          <span className="text-white/50 uppercase tracking-widest text-sm">FAQ</span>
          <h2 className="font-heading text-4xl text-white mt-2 mb-4">Heb Je Vragen?</h2>
          <p className="text-white/60">Hier vind je antwoorden op de meest gestelde vragen. Staat jouw vraag er niet bij? Neem dan gerust contact op.</p>
        </div>
            ...
            <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}>
              <button onClick={() => setOpen(open === i ? null : i)} className="w-full text-left border-b border-white/10 pb-4 mb-4">
                <div className="flex items-center gap-3">
                  <FaChevronRight className={`text-white/50 text-xs transition-transform ${open === i ? 'rotate-90' : ''}`} />
                  <span className="text-white">{f.q}</span>
                </div>
                <AnimatePresence>
                  {open === i && (
                    <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="text-white/60 text-sm mt-3 pl-6">{f.a}</motion.p>
                  )}
                </AnimatePresence>
              </button>
            </motion.div>
          ...
          {faqs.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-surface p-6 rounded-xl border border-white/5">
              <h3 className="text-white font-semibold mb-3 flex items-start gap-2">
                <span className="text-white/50 font-heading text-xl">{String(i + 1).padStart(2, '0')}</span>
                {f.q}
              </h3>
              <p className="text-white/60 text-sm pl-8">{f.a}</p>
            </motion.div>
          ))}
        </div>
      </div>
    ...
    <section className="bg-primary py-16">
      <div className="max-w-4xl mx-auto px-8">
        ...
        <div className="flex flex-wrap gap-2 mb-8 justify-center">
          {faqs.map((f, i) => (
            <button key={i} onClick={() => setActive(i)} className={`px-4 py-2 rounded-full text-sm transition ${active === i ? 'bg-white text-black font-semibold' : 'bg-surface text-white/60 hover:text-white'}`}>
              Vraag {i + 1}
            </button>
          ))}
        </div>
        <AnimatePresence mode="wait">
          <motion.div key={active} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-surface p-8 rounded-xl">
            ...
            <p className="text-white/60">{faqs[active].a}</p>
          </motion.div>
        </AnimatePresence>
      </div>
    </section>
        ...
        {faqs.map((f, i) => (
          <div key={i} className="border-b border-white/5">
            <button onClick={() => setOpen(open === i ? null : i)} className="w-full flex items-center justify-between py-6 text-left group">
              <span className="text-white group-hover:text-white transition">{f.q}</span>
              <FaChevronDown className={`text-white/50 text-sm transition-transform ${open === i ? 'rotate-180' : ''}`} />
            </button>
            <AnimatePresence>
              {open === i && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <p className="text-white/60 text-sm pb-6">{f.a}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-x-16 gap-y-8">
          {faqs.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: i * 0.1 }}>
              ...
              <p className="text-white/60 text-sm pl-4">{f.a}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-8 space-y-4">
        ...
        {faqs.map((f, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="space-y-2">
            <div className="bg-white text-black px-5 py-3 rounded-2xl rounded-bl-none inline-block font-medium text-sm">{f.q}</div>
            <div className="bg-surface text-white/80 px-5 py-3 rounded-2xl rounded-tl-none ml-8 text-sm">{f.a}</div>
          </motion.div>
        ))}
      </div>
    </section>
        ...
        {faqs.map((f, i) => (
          <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="border-b border-white/5 py-8">
            <button onClick={() => setOpen(open === i ? null : i)} className="w-full flex items-start gap-6 text-left">
              <span className="text-white/30 font-heading text-5xl">{String(i + 1).padStart(2, '0')}</span>
              <div className="flex-1">
                <h3 className="text-white font-heading text-xl">{f.q}</h3>
                <AnimatePresence>
                  {open === i && (
                    <motion.p initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="text-white/60 mt-3 overflow-hidden">{f.a}</motion.p>
                  )}
                </AnimatePresence>
              </div>
            </button>
    ...
    <section className="bg-primary py-16">
      <div className="max-w-3xl mx-auto px-8">
        ...
        <div className="relative mb-10">
          <input placeholder="Zoek in onze FAQ..." className="w-full bg-surface border border-white/10 rounded-full px-6 py-3 text-white placeholder:text-white/40 focus:border-white/50 outline-none" />
          ...
          {faqs.map((f, i) => (
            <div key={i} className="bg-surface rounded-lg">
              <button onClick={() => setOpen(open === i ? null : i)} className="w-full flex items-center justify-between p-4 text-left">
                ...
                <FaChevronDown className={`text-white/50 text-xs transition-transform ${open === i ? 'rotate-180' : ''}`} />
              </button>
              <AnimatePresence>
                {open === i && (
                  <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                    <p className="px-4 pb-4 text-white/60 text-sm">{f.a}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
    ...
    <section className="bg-primary py-16">
      <div className="max-w-7xl mx-auto px-8">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {faqs.map((f, i) => (
            <motion.div key={i} whileHover={{ scale: 1.02 }} className="bg-surface p-6 rounded-xl cursor-pointer group border border-transparent hover:border-white/30 transition-colors">
              <h3 className="text-white font-semibold group-hover:text-white transition-colors">{f.q}</h3>
              <p className="text-white/60 text-sm mt-2 max-h-0 group-hover:max-h-40 overflow-hidden transition-all duration-500">{f.a}</p>
            </motion.div>
          ))}
        </div>
      </div>
  ...
  return (
    <main className="min-h-screen bg-primary pt-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        ...
        <h1 className="font-heading text-4xl md:text-5xl text-white mt-4">FAQ <span className="text-white/50">Varianten</span></h1>
        <p className="text-white/60 mt-2">10 unieke FAQ secties om uit te kiezen</p>
      </div>
      {variants.map((v, i) => (
